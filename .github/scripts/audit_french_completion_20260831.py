...
# Do not flag valid French Â/Âge. Keep only high-confidence mojibake/corruption markers.
...
def walk(source: Any, target: Any, path: str, blockers: list[str], warnings: list[str]) -> None:
    ...
    if isinstance(source, dict):
        ...
        for key in source:
            if key in target: walk(source[key], target[key], f'{path}.{key}', blockers, warnings)
        return
    ...
    if CYRILLIC.search(target): blockers.append(f'{path}: Cyrillic text remains: {target!r}')
    ...
    for pattern, reason in BAD_GLOBAL:
        ...
        if ('cavalier' in reason or 'pilote' in reason) and not re.search(r'\briders?\b', low_source): continue
        ...
        if 'Save' in reason and not re.search(r'\bsav(?:e|es|ed|ing)\b', low_source): continue
    ...
    if re.search(r'\braces?\b', low_source) and re.search(r'(?<!\w)races?(?!\w)', semantic_target, re.I):
        blockers.append(f'{path}: cycling race left/mistranslated as race instead of course: {target!r}')
    if re.search(r'\bstages?\b', low_source) and re.search(r'\bscène(?:s)?\b|\bétage(?:s)?\b', semantic_target, re.I):
    ...
    if re.search(r'\bmanagers?\b', low_source) and not re.search(r'\badmins?|administrators?\b', low_source):
        if re.search(r'\badministrateur(?:s|rice|rices)?\b', semantic_target, re.I):
...
def main() -> None:
    ...
    for name in en_files:
        target = FR_DIR / name
        if target.exists(): walk(load(EN_DIR / name), load(target), name, blockers, warnings)
    ...
    for needle in ["code: 'fr'", "label: 'Français'", "flag: '🇫🇷'", "countryCode: 'FR'", "htmlLang: 'fr'", "locale: 'fr-FR'"]:
        if needle not in languages: blockers.append(f'French language metadata incomplete: {needle}')
    supported = re.search(r"supportedLngs:\s*\[([^\]]+)\]", index)
    if not supported or not re.search(r"['\"]fr['\"]", supported.group(1)): blockers.append('French missing from supportedLngs')
    if "language?.startsWith('fr')" not in bridge or "return 'fr-FR'" not in bridge: blockers.append('French locale date formatting missing')
    if "fr: 'fr'" not in selector: blockers.append('French flag mapping missing')

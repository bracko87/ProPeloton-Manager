...
export default function CalendarPage(): JSX.Element {
  ...
  const premiumCalendarInsights = useMemo(() => {
    const sponsorTargetRaces = activeMonthRaces.filter(
      race => (sponsorObjectiveTargetsByRaceId[race.id]?.length ?? 0) > 0
    )
    const acceptedRaces = activeMonthRaces.filter(
      race => race.existing_application_status?.toLowerCase() === 'accepted'
    )
    const openApplicationRaces = activeMonthRaces.filter(
      race => getBaseRaceCalendarStatus(race)?.toLowerCase() === 'open'
    )

    const priorityRaces = [
      ...sponsorTargetRaces,
      ...acceptedRaces.filter(
        race => !sponsorTargetRaces.some(target => target.id === race.id)
      ),
      ...openApplicationRaces.filter(
        race =>
          !sponsorTargetRaces.some(target => target.id === race.id) &&
          !acceptedRaces.some(accepted => accepted.id === race.id)
      ),
    ].slice(0, 3)

    return {
      sponsorTargetCount: sponsorTargetRaces.length,
      acceptedCount: acceptedRaces.length,
      openApplicationCount: openApplicationRaces.length,
      priorityRaces,
    }
  }, [activeMonthRaces, sponsorObjectiveTargetsByRaceId])

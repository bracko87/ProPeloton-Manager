 ...
 *
 * UPDATE: Public information pages
...
import RioStage1ShadowDiagnostic from './pages/dev/RioStage1ShadowDiagnostic'
import RioStage1LegacyComparisonDiagnostic from './pages/dev/RioStage1LegacyComparisonDiagnostic'
import RioStage1ReplayAdapterDiagnostic from './pages/dev/RioStage1ReplayAdapterDiagnostic'
...
try {
  ...
  console.error(
    'Team ranking configuration validation failed:',
    error,
  )
}
...
function RequireClub({
  ...
  useEffect(() => {
    ...
    const checkClub = async (): Promise<void> => {
      if (!user) {
        if (mounted) {
          setChecking(false)
        }
      ...
      if (rpcError) {
        setError(
          'Unable to load your club at the moment. Please try again shortly.',
        )
        setChecking(false)
        return
      }
  ...
  if (loading || checking) {
    return (
      <LoadingScreen label="Loading your club..." />
    )
  }

  if (error) {
    return (
      <GuardErrorScreen message={error} />
    )
  }
    ...
    <AuthProvider>
      <HashRouter>
        <Routes>
          ...
          <Route
            path="/dev/rio-stage-1-shadow"
            element={
              <RioStage1ShadowDiagnostic />
            }
          />

          <Route
            path="/dev/rio-stage-1-legacy-comparison"
            element={
              <RioStage1LegacyComparisonDiagnostic />
            }
          />

          <Route
            path="/dev/rio-stage-1-replay-adapter"
            element={
              <RioStage1ReplayAdapterDiagnostic />
            }
          />

          <Route
            path="/dev/rio-stage-1-replay-controller"
            element={
              <RioStage1ReplayControllerDiagnostic />
            }
          />

          <Route
            path="/dev/rio-stage-1-generic-replay-view"
            element={
              <RioStage1GenericReplayViewDiagnostic />
            }
          />
           ...
           * Create club route.
           *
           * This route is intentionally outside /dashboard and does not use the
           * old RequireNoClub guard. That old guard checked get_my_club_id(),
           * which can still return a liquidated club and redirect the user back
           * to dashboard.
           */}
          <Route
            path="/create-club"
          ...
          >
            ...
            <Route
              path="riders/:riderId"
              element={<RiderProfilePublicPage />}
            />

            <Route
              path="my-riders/:riderId"
              element={<RiderProfileOwnPage />}
            />

            <Route
              path="external-riders/:riderId"
              element={<ExternalRiderProfilePage />}
            />

            <Route
              path="compare-riders"
              element={<CompareRidersPage />}
            />

/**
 * runCalibratedTerrainSeparationStageWithRaceIncidents.ts
 *
 * Explicit development-only calibrated wrapper with one shared individual
 * crash, group crash, and technical-incident selector.
 *
 * The accepted calibrated wrapper, the Phase 8H.2B individual wrapper, the
 * Phase 8H.3B crash wrapper, and the public race boundary remain unchanged.
 */
...
export interface RunCalibratedTerrainSeparationStageWithRaceIncidentsOptions {
...
export interface RunCalibratedTerrainSeparationStageWithRaceIncidentsResult
...
export function runCalibratedTerrainSeparationStageWithRaceIncidents(
  initialState:
    SimulationState,
  options:
    RunCalibratedTerrainSeparationStageWithRaceIncidentsOptions = {},
): RunCalibratedTerrainSeparationStageWithRaceIncidentsResult {
  ...
  if (
    ...
    throw new Error(
      'runCalibratedTerrainSeparationStageWithRaceIncidents: maximumTickCount must be a positive integer.',
    )
  }
  ...
  for (const tick of stage.ticks) {
    ...
    for (
      ...
      if (
        ...
        throw new Error(
          'runCalibratedTerrainSeparationStageWithRaceIncidents: replay boundary must align with a tick boundary.',
        )
      }

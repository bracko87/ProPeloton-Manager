...
export function applyFlatScenarioV1(
  ...
  const selectionSeed=`flat-scenario:${input.race.raceId}:${input.stage.stageId}:${FLAT_SCENARIO_CATALOG_VERSION}`
  const persistedTemplateId=(options.history ?? []).find((entry)=>entry.stageId===input.stage.stageId)?.templateId ?? null

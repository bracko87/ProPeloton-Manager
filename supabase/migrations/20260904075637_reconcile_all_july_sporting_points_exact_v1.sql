...
begin
  ...
  if exists (
    ...
    where extract(month from s.stage_date)=7
      and (
    ...
    raise exception 'July reconciliation aborted: at least one stage has simulation/authoritative/official point-result activity.';
  end if;
  ...
  loop
    if not exists (select 1 from public.race_stage_profile_details p where p.stage_id=v_stage.id) then
      raise exception 'July reconciliation aborted: profile missing for stage % (% stage %).', v_stage.id, v_stage.race_name, v_stage.stage_number;
    end if;
    ...
    if not coalesce((v_before->>'visible_sources_match')::boolean,false) then
      raise exception 'July reconciliation aborted: visible stage/profile mismatch for stage % (% stage %): %', v_stage.id, v_stage.race_name, v_stage.stage_number, v_before;
    end if;
    ...
    if not coalesce((v_after->>'ready')::boolean,false) then
      raise exception 'July reconciliation failed exact readiness for stage % (% stage %). reconcile=% after=%', v_stage.id, v_stage.race_name, v_stage.stage_number, v_reconcile, v_after;
    end if;

    v_count := v_count + 1;
  end loop;

  if v_count <> (select count(*) from public.race_stages where extract(month from stage_date)=7) then
    raise exception 'July reconciliation stage-count mismatch: processed %, expected %', v_count, (select count(*) from public.race_stages where extract(month from stage_date)=7);
  end if;
end
$$;

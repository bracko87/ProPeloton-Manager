...
begin
  ...
  where upper(w.country_code)=v_source
    and w.week_of_year between 10 and 48
  ...
  if extract(year from v_start)::integer <> v_year
     or extract(year from v_final)::integer <> v_year
  then
    return jsonb_build_object(
      'status','weather_data_unavailable',
      'country_code',v_country,
      'climate_source_country_code',v_source,
      'reason','selected_window_outside_season_year'
    );
  end if;

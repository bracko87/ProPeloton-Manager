-- Route any Pass 2 worker lease that has been stale for seven minutes to the
-- proven emergency engine on the next claim. This prevents heavy stages from
-- endlessly rerunning the same primary calculation after an Edge runtime loss.

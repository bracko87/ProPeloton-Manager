-- Ensure failed scenario reservations are reactivated on retry instead of
-- falling through to a duplicate INSERT after intervening queries overwrite
-- PostgreSQL's special FOUND flag.
...
begin
  ...
  if v_existing_found and v_existing.selection_status in ('reserved','completed') then
  ...
  insert into public.race_engine_scenario_runs(
    ...
    context_snapshot_json,candidate_scores_json,repetition_penalties_json,generated_parameters_json,
    applied_directives_json,selection_status
  ) values (

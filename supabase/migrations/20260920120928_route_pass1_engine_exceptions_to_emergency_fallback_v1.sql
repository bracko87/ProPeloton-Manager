...
comment on function public.universal_race_stage_claim_pass1_resume_v1() is
'Claims resumable Pass 1 work but leaves deterministic Pass 1 engine exceptions for emergency Pass 2 fallback instead of retrying them indefinitely.';

comment on function public.universal_race_stage_claim_pass2_resume_v1() is
'Claims normal Pass 2 work and converts Pass 1 watchdog timeouts or deterministic Pass 1 engine exceptions into emergency fallback runs so one bad stage cannot monopolize the race queue.';

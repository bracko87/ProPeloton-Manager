-- Persist the production hotfix applied during the 2026-09-17 race-engine incident.
-- Keep the live-state polling path off the multi-megabyte result_summary_json payload.
...
AS $function$
  ...
  latest_run as (
    select run.id as simulation_run_id, run.status as run_status, run.created_at
  ...
  resolved as (
    select
      ...
      coalesce(lifecycle.replay_closes_at_real, latest_run.created_at + interval '15 minutes') as live_ends_at
    from (select 1) seed
  ...
  select jsonb_build_object(
    ...
    'is_live', case
      ...
      when resolved.last_status = 'replay_live' and resolved.live_ends_at is not null then now() < resolved.live_ends_at
      else false
    end,

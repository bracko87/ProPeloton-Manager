...
begin
  ...
  loop
    ...
    values (
      ...
      v_inventory.id,
      coalesce(nullif(v_entry.value ->> 'teamId','')::uuid, v_inventory.club_id),
      v_inventory.equipment_category,
      v_loss,
      v_stage_date,
      jsonb_build_object(
        'source', 'phase11_equipment_wear_compat_v2',
        'reason', 'canonical application manifest omitted equipment resources',
  ...
  return jsonb_build_object(
    'ok', true,
    'status', case when v_candidates = 0 then 'no_equipment_candidates' else 'processed' end,
    'simulation_run_id', p_simulation_run_id,
    'stage_id', v_stage_id,
...
declare
  ...
  v_def text;
  v_new text;
  v_old text := 'v_wear_result := public.race_engine_apply_stage_equipment_asset_wear_v1(v_run.id, false);';
  v_replacement text := E'v_wear_result := public.race_engine_apply_stage_equipment_asset_wear_v1(v_run.id, false);\n  v_wear_result := coalesce(v_wear_result, ''{}''::jsonb) || jsonb_build_object(\n    ''equipment_compat'',\n    public.race_engine_apply_missing_stage_equipment_wear_v2(v_run.id)\n  );';
begin
  select p.oid
  into v_oid
  from pg_proc p
  ...
  where n.nspname = 'public'
    and p.proname = 'universal_race_stage_finalize_core_survival_v1'
  order by p.oid desc
  limit 1;

  if v_oid is null then
    raise exception 'universal_race_stage_finalize_core_survival_v1 not found';
  end if;

  v_def := replace(pg_get_functiondef(v_oid), E'\r\n', E'\n');

  if strpos(v_def, v_old) = 0 then
    raise exception 'equipment wear closeout patch point not found';
  end if;

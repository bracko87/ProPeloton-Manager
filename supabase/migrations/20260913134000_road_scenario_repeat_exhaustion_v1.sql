-- Road Scenario V1 repeat-exhaustion correction.
-- Allows an exact scenario template to repeat only after every currently
-- compatible template of the same road category has already been used in the
-- relevant race/day scope. This keeps hilly and flat behavior symmetrical even
-- when an audit does not explicitly carry repeatAllowed* booleans.

...
begin
  ...
  from (
    ...
    join public.race_stage_results winner on winner.stage_id = stage.id and winner.rank = 1
    left join public.riders winner_rider on winner_rider.id = winner.rider_id
    left join public.race_classification_standings gc
      on gc.race_id = race.id and gc.after_stage_id = stage.id
     and gc.classification_type = 'general' and gc.entity_type = 'rider' and gc.rank = 1
    left join public.race_classification_standings mountain
      on mountain.race_id = race.id and mountain.after_stage_id = stage.id
     and mountain.classification_type = 'mountain' and mountain.entity_type = 'rider' and mountain.rank = 1
    left join public.race_classification_standings points
      on points.race_id = race.id and points.after_stage_id = stage.id
     and points.classification_type = 'points' and points.entity_type = 'rider' and points.rank = 1
...
revoke all on function public.get_overview_recent_race_results_v1() from public, anon;
grant execute on function public.get_overview_recent_race_results_v1() to authenticated, service_role;

comment on function public.get_overview_recent_race_results_v1() is
'Returns recent stage winners, stage identity/route and GC, mountain and points leaders for the Overview carousel.';

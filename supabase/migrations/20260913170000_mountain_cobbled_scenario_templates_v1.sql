-- Road Scenario Template System V1: Mountain + Cobbled expansion.
-- Extends the authoritative Flat/Hilly scenario reservation contract to
-- Mountain V1 and Cobbled V1 while preserving existing selections/history
-- and the production failed-run reactivation behavior.

alter table public.race_engine_scenario_runs
  drop constraint if exists race_engine_scenario_runs_type_check;

alter table public.race_engine_scenario_runs
  add constraint race_engine_scenario_runs_type_check
  check (scenario_type in ('flat','hilly','mountain','cobbled'));
...
begin
  ...
  -- Existing authoritative/reserved selection remains immutable for the stage.
  ...
  if v_compatible_count > 0 then
    ...
    if v_race_used_compatible >= v_compatible_count then
      v_allow_race_repeat := true;
    end if;

    if v_day_used_compatible >= v_compatible_count then
      v_allow_day_repeat := true;
    end if;
  end if;
  ...
  if not v_allow_race_repeat then
    ...
    if v_race_collision is not null then
      return jsonb_build_object(
        ...
        'conflicting_stage_id',v_race_collision
      );
    end if;
  end if;
  ...
  if not v_allow_day_repeat then
    ...
    if v_day_collision is not null then
      return jsonb_build_object(
        ...
        'scenario_type',v_type,
        'template_id',v_template_id,
        'conflicting_stage_id',v_day_collision
      );
    end if;
  end if;

  -- Preserve the production retry path: a failed attempt may reactivate the
  -- stage's scenario row for a fresh simulation run without deleting history.
  if found and v_existing.selection_status='failed' then
    ...
    set simulation_run_id=p_simulation_run_id,
        ...
        scenario_deviations_json=coalesce(v_existing.scenario_deviations_json,'[]'::jsonb)
          || jsonb_build_array(
               jsonb_build_object(
                 'kind','scenario_retry_reactivated',
                 'at',clock_timestamp()
               )
             ),
        updated_at=clock_timestamp()
  ...
  insert into public.race_engine_scenario_runs(
    ...
    repetition_penalties_json,
    generated_parameters_json,
    applied_directives_json,
    selection_status
  ) values (
    p_simulation_run_id,
    v_stage.race_id,
    ...
    coalesce(p_audit->'appliedDirectives','{}'::jsonb),
    'reserved'
  );

...
do $$
begin
  if to_regprocedure('public._get_club_squad_season_dashboard_v1_internal(uuid,integer)') is null
     and to_regprocedure('public.get_club_squad_season_dashboard_v1(uuid,integer)') is not null then
    alter function public.get_club_squad_season_dashboard_v1(uuid, integer)
      rename to _get_club_squad_season_dashboard_v1_internal;
  end if;
end
$$;
...
revoke all on function public._get_club_squad_season_dashboard_v1_internal(uuid, integer)
